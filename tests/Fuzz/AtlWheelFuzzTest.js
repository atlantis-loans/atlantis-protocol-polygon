...
describe.skip('AtlantisWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Atlantis protocol
  ...
  let initialState = globals => {
    return {
      ...
      atlantisSupplySpeed: new bn(1),
      atlantisSupplyIndex: new bn(1),
      atlantisSupplyIndexSnapshots: {},
      atlantisSupplyIndexUpdatedBlock: globals.blockNumber,

      atlantisBorrowSpeed: new bn(1),
      atlantisBorrowIndex: new bn(1),
      atlantisBorrowIndexSnapshots: {},
      atlantisSupplyIndexUpdatedBlock: globals.blockNumber,

      atlantisAccruedWithCrank: {}, // naive method, accruing all accounts every block
      atlantisAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see atlantis distributed. just for fuzz testing.
  ...
  let trueUpAtl = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.atlantisSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.atlantisBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates atlantis accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        atlantisAccruedWithCrank[account] = get(
          state.atlantisAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(atlantisSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      atlantisAccruedWithCrank: atlantisAccruedWithCrank,
    };
  };

  // real deal atlantis index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      atlantisBorrowSpeed,
      atlantisBorrowIndex,
      atlantisBorrowIndexSnapshots,
      atlantisAccruedWithIndex,
      totalBorrows,
      borrowBalances,
      atlantisBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(atlantisBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      atlantisBorrowIndex = atlantisBorrowIndex.plus(
        atlantisBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = atlantisBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      atlantisBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      atlantisAccruedWithIndex[account] = get(atlantisAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(atlantisBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      atlantisBorrowIndexUpdatedBlock: globals.blockNumber,
      atlantisBorrowIndex: atlantisBorrowIndex,
      atlantisBorrowIndexSnapshots: {
        ...state.atlantisBorrowIndexSnapshots,
        [account]: atlantisBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      atlantisAccruedWithIndex,
      totalSupply,
      atlantisSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(atlantisSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      atlantisSupplyIndex = atlantisSupplyIndex.plus(
        atlantisSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = atlantisSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some atlantis
      atlantisAccruedWithIndex[account] = get(atlantisAccruedWithIndex[account]).plus(
        balances[account].times(atlantisSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      atlantisSupplyIndexUpdatedBlock: globals.blockNumber,
      atlantisSupplyIndex: atlantisSupplyIndex,
      atlantisSupplyIndexSnapshots: {
        ...state.atlantisSupplyIndexSnapshots,
        [account]: atlantisSupplyIndex
      },
      atlantisAccruedWithIndex: atlantisAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * atlantis speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.atlantisSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.atlantisBorrowSpeed));

    let actual = Object.values(state.atlantisAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed atlantis inaccurately`
    );
  };

  // assert atlantis distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.atlantisAccruedWithCrank;
    let actual = state.atlantisAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpAtlantis(globals, state);
  };
