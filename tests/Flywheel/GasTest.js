...
describe.skip('Flywheel trace ops', () => {
  ...
  beforeEach(async () => {
    ...
    await send(comptroller, '_addAtlantisMarkets', [[market].map(c => c._address)]);
  });
  ...
  it('update supply index SSTOREs', async () => {
    ...
    await send(comptroller, 'setAtlantisSpeed', [market._address, etherExp(0.5)]);

    const tx = await send(comptroller, 'harnessUpdateAtlantisSupplyIndex', [market._address]);
  ...
  it('update borrow index SSTOREs', async () => {
    ...
    const tx = await send(comptroller, 'harnessUpdateAtlantisBorrowIndex', [market._address, etherExp(1.1)]);
