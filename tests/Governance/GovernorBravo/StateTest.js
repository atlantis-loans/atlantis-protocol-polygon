...
describe('GovernorBravo#state/1', () => {
  let atlantis, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorBravoImmutable', [timelock._address, atlantis._address, root, 17280, 1, "65000000000000000000000"]);
    await send(gov, '_initiate');
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(atlantis, 'transfer', [acct, etherMantissa(400000)]);
    await send(atlantis, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(atlantis, 'transfer', [accounts[0], etherMantissa(400000)]);
    await send(atlantis, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(atlantis, 'delegate', [root], { from: accounts[0] });
