...
async function makeAToken(opts = {}) {
  ...
  const symbol = opts.symbol || (kind === 'MATIC' ? 'aMATIC' : 'aOMG');
  ...
  switch (kind) {
    case 'abnb':
      aToken = await deploy('AMATICHarness',
        [
...
async function setEtherBalance(aMATIC, balance) {
  const current = await etherBalance(aMATIC._address);
  const root = saddle.account;
  expect(await send(aMATIC, 'harnessDoTransferOut', [root, current])).toSucceed();
  expect(await send(aMATIC, 'harnessDoTransferIn', [root, balance], { value: balance })).toSucceed();
}
