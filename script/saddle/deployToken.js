...
note: pass VERIFY=true and MATICERSCAN_API_KEY=<api key> to verify contract on Etherscan
...
(async function() {
  ...
  if (env['VERIFY']) {
    const etherscanApiKey = env['MATICERSCAN_API_KEY'];
    if (etherscanApiKey === undefined || etherscanApiKey.length === 0) {
      throw new Error(`MATICERSCAN_API_KEY must be set if using VERIFY flag...`);
    }
