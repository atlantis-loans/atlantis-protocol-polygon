...
export async function buildMaximillion(world: World, from: string, event: Event): Promise<{world: World, maximillion: Maximillion, maximillionData: MaximillionData}> {
  const fetchers = [
    new Fetcher<{aMATIC: AddressV}, MaximillionData>(`
        #### Maximillion
      ...
      async (world, {aMATIC}) => {
        return {
          invokation: await MaximillionContract.deploy<Maximillion>(world, from, [aMATIC.val]),
          description: "Maximillion",
          aMATICAddress: aMATIC.val
        };
      },
      {catchall: true}
    )
