...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, atlantis: AddressV, admin: AddressV, implementation: AddressV, votingPeriod: NumberV, votingDelay: NumberV, proposalThreshold: NumberV},
      GovernorBravoData
    >(
      `
      #### GovernorBravoDelegator

      * "GovernorBravo Deploy BravoDelegator name:<String> timelock:<Address> atlantis:<Address> admin:<Address> implementation<address> votingPeriod:<Number> votingDelay:<Number>" - Deploys Atlantis Governor Bravo with a given parameters
        * E.g. "GovernorBravo Deploy BravoDelegator GovernorBravo (Address Timelock) (Address Atlantis) Admin (Address impl) 17280 1"
    `,
      "BravoDelegator",
      [
      ...
      async (world, { name, timelock, atlantis, admin, implementation, votingPeriod, votingDelay, proposalThreshold }) => {
        return {
          invokation: await GovernorBravoDelegator.deploy<GovernorBravo>(
            world,
            from,
            [timelock.val, atlantis.val, admin.val, implementation.val, votingPeriod.encode(), votingDelay.encode(), proposalThreshold.encode()]
          ),
          name: name.val,
    ...
    new Fetcher<
      { name: StringV, timelock: AddressV, atlantis: AddressV, admin: AddressV, votingPeriod: NumberV, votingDelay: NumberV, proposalThreshold: NumberV },
      GovernorBravoData
    >(
      `
      #### GovernorBravoImmutable

      * "GovernorBravoImmut Deploy BravoImmutable name:<String> timelock:<Address> atlantis:<Address> admin:<Address> votingPeriod:<Number> votingDelay:<Number>" - Deploys Atlantis Governor Bravo Immutable with a given parameters
        * E.g. "GovernorBravo Deploy BravoImmutable GovernorBravo (Address Timelock) (Address Atlantis) Admin 17280 1"
    `,
      "BravoImmutable",
      [
        new Arg("name", getStringV),
      ...
      async (world, { name, timelock, atlantis, admin, votingPeriod, votingDelay, proposalThreshold }) => {
        return {
          invokation: await GovernorBravoImmutable.deploy<GovernorBravo>(
            world,
            from,
            [timelock.val, atlantis.val, admin.val, votingPeriod.encode(), votingDelay.encode(), proposalThreshold.encode()]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy BravoDelegate name:<String>" - Deploys Atlantis Governor Bravo Delegate
    ...
    >(
      ...
      * "Governor Deploy BravoDelegateHarness name:<String>" - Deploys Atlantis Governor Bravo Delegate Harness
