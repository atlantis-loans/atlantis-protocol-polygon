...
import { getAtlantisValue, atlantisFetchers } from './Value/AtlantisValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isAtlantisKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isAtlantisStr = await world.web3.eth.getStorageAt(addr.val, isAtlantisKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isAtlantisStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Atlantis ...atlantisArgs" - Returns Atlantis value
    `,
    'Atlantis',
    [new Arg('res', getAtlantisValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: atlantisFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "AtlantisLens", implicit: false },
